...
with open("ott/__init__.py", "a") as f: pass # Ensure it's a package
...
async def is_authorized(callback_query: CallbackQuery):
    ...
    if callback_query.message.reply_to_message:
        ...
        await callback_query.answer("⚠️ Original message lost! Kripya naya search karein.", show_alert=True)
        return False
...
# 🚀 DYNAMIC OTT HANDLER (/nf, /prime, /zee5, etc.)
...
if valid_commands:
    ...
    async def dynamic_ott_cmd(client: Client, message: Message):
        ...
        if len(message.command) < 2:
            return await message.reply_text(f"⚠️ Kripya URL dein. Example: `/{cmd} <url>`", reply_to_message_id=message.id)
        
        url = message.text.split(" ", 1)[1].strip()
        msg = await message.reply_text(f"🔄 **{cmd.upper()}** link scrape kar raha hoon... ⏳", reply_to_message_id=message.id)
        
        try:
            module = ott_modules[cmd]
            data = module.scrape(url) if hasattr(module, "scrape") else None
            
            if data:
                user_mention = message.from_user.mention if message.from_user else "Admin"
                title = data.get("title", "Unknown Title")
                main_poster = data.get("main_poster", "")
                portrait = data.get("portrait", "")
                cover = data.get("cover", "")
                
                caption = (
                    f"{user_mention}\n`/{cmd} {url}`\n\n**{cmd.upper()} Poster:**\n{main_poster}\n\n"
                    f"**Portrait:** [Click Here]({portrait})\n\n**Cover:** [Click Here]({cover})\n\n**{title}**"
                )
                btn = InlineKeyboardMarkup([[InlineKeyboardButton("📢 Update Channel", url=UPDATE_CHANNEL_URL)]])
                await message.reply_photo(photo=main_poster, caption=caption, reply_markup=btn, reply_to_message_id=message.id)
                await msg.delete()
...
async def show_main_options(client: Client, callback_query: CallbackQuery):
    ...
    s_type, m_id = callback_query.data.split("_")[1], callback_query.data.split("_")[2]
...
async def show_sub_options(client: Client, callback_query: CallbackQuery):
    if not await is_authorized(callback_query): return
    cat, s_type, m_id = callback_query.data.split("_")[1], callback_query.data.split("_")[2], callback_query.data.split("_")[3]
...
async def paginate_images(client: Client, callback_query: CallbackQuery):
    ...
    if flt == "txt": images_list = [img for img in raw_images if img.get('iso_639_1') is not None and img.get('iso_639_1') != "xx"]
    elif flt == "cln": images_list = [img for img in raw_images if img.get('iso_639_1') is None or img.get('iso_639_1') == "xx"]
